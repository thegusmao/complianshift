...
class Scanner:
    ...
    def _get_csvs(self, cache_minutes, force, console, debug):
        csv_cache_path = self.data_dir / "csvs-report.json"
        if self._is_cache_valid(csv_cache_path, cache_minutes, force):
            if console:
                console.print(f"[dim]Loading CSVs from local cache ({csv_cache_path})...[/dim]")
            with open(csv_cache_path, "r") as f:
                return json.load(f)
                
        if console:
            console.print("[dim]Fetching Red Hat Operator ClusterServiceVersions (CSVs) (this may take a while in large clusters)...[/dim]")
            
        csvs = self.k8s.get_redhat_csvs(console=console, debug=debug)
        
        with open(csv_cache_path, "w") as f:
            json.dump(csvs, f, indent=2)
        if console and debug:
            console.print(f"[dim]CSVs saved to cache: {csv_cache_path}[/dim]")
            
        return csvs

    def _deduplicate_operators(self, csvs, console):
        unique_operators = {}
        for csv in csvs:
            display_name = csv["display_name"]
            namespace = csv["namespace"]
            
            if display_name not in unique_operators:
                unique_operators[display_name] = csv
            else:
                # Preferência por cluster-scoped (openshift-operators)
                if namespace == "openshift-operators":
                    unique_operators[display_name] = csv
                    
        if console:
            console.print(f"[dim]Found {len(unique_operators)} unique Red Hat operators.[/dim]")
        return unique_operators

    def _resolve_api_name(self, display_name, package):
        api_name = self.mapping.get(display_name)
        if not api_name:
            api_name = self.mapping.get(package)
        return api_name or display_name
    ...
    def _find_matching_version(self, version_clean, versions, console, debug):
        ...
        major_v = v_parts[0] if len(v_parts) >= 1 else version_clean
    ...
    def _build_result(self, name, csv, compat, status, end_date):
        return {
            "name": name,
            "scope": csv["scope"],
            "namespace": csv["namespace"],
    ...
    def scan_operators(self, cache_minutes=30, force=False, console=None, debug=False):
        ...
        csvs = self._get_csvs(cache_minutes, force, console, debug)
        unique_operators = self._deduplicate_operators(csvs, console)
        lifecycle_data = self.load_lifecycle_data()
        
        results = []
        
        for display_name, csv in unique_operators.items():
            if console:
                console.print(f"\n[bold cyan]Analyzing operator:[/bold cyan] {display_name}")
                
            version = csv["version"]
            version_clean = version[1:] if version.startswith("v") else version
            
            api_name = self._resolve_api_name(display_name, csv["package"])
            if console and debug:
                console.print(f"[dim]Looking for product: '{api_name}' (Installed version: {version})[/dim]")
            ...
            if not product_info:
                ...
                results.append(self._build_result(display_name, csv, "N/A", "Unknown", "N/A"))
                continue
                
            matched_version = self._find_matching_version(version_clean, product_info.get("versions", []), console, debug)
            if not matched_version:
                if console:
                    console.print(f"[yellow]Compatible version for '{version}' not found for '{api_name}'.[/yellow]")
                results.append(self._build_result(display_name, csv, "N/A", "Version not found", "N/A"))
                continue
            ...
            results.append(self._build_result(display_name, csv, ocp_compatible, support_status, end_date))
